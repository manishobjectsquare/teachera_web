...
const EditQuizQuestionModal = ({ isOpen, onClose, questionId }) => {
    ...
    const fetchQuestionData = async () => {
      ...
      try {
        const res = await axios.get(
          `${baseUrl}/api/v1/admin/quiz-question/quiz-question-view/${questionId}`
        );
  ...
  const handleOptionChange = (id, value) => {
    setFormData((prev) => ({
      ...
      options: prev.options.map((opt) =>
        opt._id === id ? { ...opt, option_text: value } : opt
      ),
    }));
  };
  ...
  const handleCorrectToggle = (id) => {
    setFormData((prev) => ({
      ...prev,
      options: prev.options.map((opt) =>
        opt._id === id ? { ...opt, is_correct: !opt.is_correct } : opt
      ),
    }));
  };
  ...
  const addOption = () => {
    const newId = Math.max(...formData.options.map((o) => o._id)) + 1;
    setFormData((prev) => ({
      ...prev,
      options: [
        ...prev.options,
  ...
  const removeOption = (id) => {
    ...
    setFormData((prev) => ({
      ...prev,
      options: prev.options.filter((opt) => opt._id !== id),
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(
        `${baseUrl}/api/v1/admin/quiz-question/quiz-question-edit/${questionId}`,
        {
          method: "PUT",
          headers: {
      ...
      if (result.status) {
        ...
        alert(result.message || "Failed to update quiz question");
      }
    } catch (error) {
      console.error("API error:", error);
      alert("Something went wrong while updating quiz question");
    }
  };
                  ...
                  {formData.options.map((opt, i) => (
                    <div key={i} className="mb-3 d-flex align-items-center">
                      <input
                        ...
                        onChange={(e) =>
                          handleOptionChange(opt._id, e.target.value)
                        }
                        required
                      />
                      <label className="form-check-label me-2">
                        <input
                          ...
                          onChange={() => handleCorrectToggle(opt._id)}
                        />{" "}
                        Correct
                      </label>
                      <button
