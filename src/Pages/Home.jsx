...
const HomePage = () => {
  ...
  const fetchCourses = async () => {
    try {
      const response = await fetch(
        `https://api.basementex.com/api/v1/web/course-list?language_type=${i18n.language == "ar" ? "arabic" : "english"
        }`
      );
  ...
  const fetchInstructors = async () => {
    try {
      const response = await fetch("https://api.basementex.com/api/v1/web/user/user-list/role=instructor", {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
    ...
    <div className="home-page">
        ...
        <div className="container">
          <div className="heading">
            <h2 className="title text-center">
              {t("Achieve your career goals with diplomas designed for you")}
            </h2>
          </div>
          <DiplomaSlider />
          <BundleSlider />
      ...
      {/* <section className="course-sec tp-space">
        <div className="container">
          ...
          <div className='text-center mt-3'>
            <Link to="" className="thm-btn">{t('See All Diplomas')}</Link>
          </div>
        </div>
      </section> */}
      ...
      <section className="course-sec ">
        <div className="container">
              ...
              {categories.map((cat, index) => (
                <li className="tab-item" key={index}>
                  <Link
                    ...
                    role="tab"
                  >
                    {i18n?.language == "en" || cat?.arabic_name == ""
                      ? cat?.name
                      : cat?.arabic_name}
                  </Link>
                </li>
          ...
          <div className="skillshare-tab-container">
            <ul className="skillshare-tab-list justify-content-center">
              {categories.map((cat, index) => (
                <li className="skillshare-tab-item" key={index}>
                  <Link

                    className={`skillshare-pill-link ${index === 0 ? "featured-active" : ""}`}
                    to={`/courses?category=${cat?._id}`}
                    data-bs-target={`#tab-pane-0${index}`}
                    role="tab"
                  >
      ...
      <section className="blog-sec tp-space">
        ...
        <style jsx>{`
          ...
          @media (max-width: 991px) {
            ...
            /* 1. Main Container (no background, minor padding) */
.skillshare-tab-container {
...
/* 3. Individual List Item (li) - Sets the gap between pills */
.skillshare-tab-item {
  list-style: none;
  /* TIGHTER GAP: Horizontal and Vertical */
  margin: 6px 6px; 
  padding: 0;
}

/* 4. The Link/Pill Button (a) - Default State */
.skillshare-pill-link {
  display: block;
  padding: 8px 16px; /* Inner padding */
  
  /* Pill Shape and Border */
  border: 1px solid #585e68; /* Gray border */
  border-radius: 50px; 
  ...
  font-weight: 500;
  text-decoration: none;
  color: #4a4a4a; /* Dark text for contrast on a lighter page background */
...
/* 5. Hover Effect */
.skillshare-pill-link:hover {
  background-color: #f0f0f0; /* Subtle background change on hover */
  border-color: #4a4a4a;
}

/* 6. Active/Featured State (The Bright Green Pill) */
.skillshare-pill-link.featured-active {
  /* Skillshare's bright green */
  background-color: #0055D2; 
  border-color: #0055D2;
  /* Dark text for maximum contrast on the bright green */
  color: #fff; 
  font-weight: 500;
}
        `}</style>
